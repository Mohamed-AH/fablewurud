/**
 * Content report builder — pure HTML generation, no database access.
 *
 * Given already-fetched collections, returns a print-ready Arabic (RTL) HTML
 * report covering series, lectures, homepage sections, articles and the PDF
 * library, with Hijri dates wherever a date is available.
 *
 * Shared by the CLI (`scripts/generate-report.js`, which manages its own DB
 * connection) and the admin route (`GET /admin/reports/:realm`, which reuses
 * the app's connection). This module must NEVER touch the mongoose connection.
 */

const { convertToHijri } = require('./dateUtils');
 ...
 * @param {Array}  data.lectures
 * @param {Array}  data.sections
 * @param {Array}  data.sheikhs
 * @param {Array}  data.articles
 * @param {Array}  data.publications
 * @param {'najmi'|'hasan'|null} data.realm - scope the report to one scholar
 * @returns {string} HTML document
 */
function buildReportHtml({ series, lectures, sections, sheikhs, articles, publications, realm } = {}) {
  // Local copies — never mutate the caller's arrays (the web route may cache them).
  let allSeries = [...(series || [])];
  let allLectures = [...(lectures || [])];
  const allSections = sections || [];
  const allSheikhs = sheikhs || [];
  let allArticles = [...(articles || [])];
  let allPublications = [...(publications || [])];
  const realmFilterArg = (realm === 'najmi' || realm === 'hasan') ? realm : null;

  // Realm attribution (Najmi vs Hasan) by sheikh — each doc counted once.
  ...
  // Optional single-realm scope. Sections (a Hasan homepage concept) and
  // articles (Hasan-side content) are omitted for the Najmi report below.
  if (realmFilterArg) {
...
<tr><th>#</th><th>اسم السلسلة</th><th>العالِم</th><th>التصنيف</th><th>القسم</th><th>المحاضرات</th><th>أضيفت (هجري)</th><th>الحالة</th></tr>\n`;
  ...
  // Per-realm breakdown (only meaningful in the combined report).
  if (!realmFilterArg) {
