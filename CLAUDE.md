...
### 📄 Content report generator — realm-aware, articles + PDFs, Hijri, admin buttons (2026-09, commits `0627ab6`, `2903b6c`, `b4a045a`, `909672d`)
Print-ready Arabic (RTL, Cairo) content report. HTML → print to PDF in the browser.
- **Shared builder:** `utils/contentReport.js` `buildReportHtml({ series, lectures, sections, sheikhs, articles, publications, realm })` — **pure, no DB access** (never touch the mongoose connection; each caller fetches). Realm filtering + section numbering + HTML all live here. Works on copies so cached input arrays aren't mutated.
- **CLI** `scripts/generate-report.js` — fetches (own connect/disconnect) + delegates to the builder. `--realm najmi|hasan` (default both) → `report-<realm>.html`; `--env`, `--output`.
- **Admin buttons:** Manage → Quick Actions has 3 buttons (Hasan / Najmi / Combined) → `GET /admin/reports/:realm` (`najmi|hasan|combined`, `isAdmin`). Builds via the app connection, **caches HTML 5 min per realm** (`admin:report:<realm>` — full-collection read), sends `no-store`; `?print=1` injects `window.print()` on load so the browser's Save-as-PDF dialog opens (chosen over server-side puppeteer — no ~300MB dep / OOM risk on Render Hobby; browser renders Arabic perfectly).
- **Coverage:** series, lectures (by series + standalone), homepage sections, **articles** (Hasan-side; omitted for Najmi), **PDF library / publications** (realm-scoped by sheikhId, per-category counts). Stat-row boxes for مقالة / كتاب PDF when present.
- **Realm scoping:** Najmi report drops the homepage-sections block (a Hasan concept) + articles; sections renumber dynamically (Najmi: ١ سلاسل · ٢ مكتبة · ٣ محاضرات). `العالِم` split line only in the combined report. Titles/footer realm-aware.
- **Hijri everywhere a date exists** (`utils/dateUtils.convertToHijri`, moment-hijri): report date subtitle+footer **lead with Hijri** (Arabic-indic) + Gregorian in parens; lectures `التاريخ (هجري)` (stored `dateRecordedHijri` else converted); series `أضيفت (هجري)` (createdAt); articles published date; publications added date.
- **Note:** combined report pulls ALL lectures (~4,800+) into one page — large tab / multi-page PDF (expected). 5-min cache means freshly-added content can lag up to 5 min in the report.
